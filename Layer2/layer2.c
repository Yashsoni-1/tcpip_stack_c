...
arp_table_lookup(arp_table_t *arp_table, char *ip_addr)
{
    ...
    ITERATE_GLTHREAD_BEGIN(&arp_table->arp_entries, curr) {
        ...
        if (strncpy(ip_addr, arp_entry->ip_addr.ip_addr, 16) == 0)
            return arp_entry;
...
bool_t
arp_table_entry_add(arp_table_t *arp_table, arp_entry_t *arp_entry_new)
{
    arp_entry_t *arp_entry_old = arp_table_lookup(arp_table, arp_entry_new->ip_addr.ip_addr);
    
    if(arp_entry_old && memcmp(arp_entry_old, arp_entry_new, sizeof(arp_entry_t)) == 0)
        return FALSE;
    ...
    init_glthread(&arp_entry_new->arp_glue);
    glthread_add_next(&arp_table->arp_entries, &arp_entry_new->arp_glue);
...
void send_arp_broadcast_req(node_t *node, interface_t *oif, char *ip_addr)
{
...
interface_set_vlan(node_t *node, interface_t *interface, unsigned int vlan_id)
{
    ...
    if(IF_L2_MODE(interface) == TRUNK)
    {
        ...
        printf("Error : Interface %s : Max Vlans membership limit reached", interface->if_name);
    }
}
                     ...
                     unsigned int pkt_size)
{
    ...
    if(outgoing_intf)
    {
        ...
        {
            assert(0);
            send_arp_broadcast_req(node, oif, next_hop_ip_str);
            return;
        }
...
layer2_pkt_recv_from_top(node_t *node,
                         ...
                         int protocol_number)
{
    assert(pkt_size < ((ethernet_hdr_t *)0)->payload);
