...
//  tcp_ip_stack_c
//
//  Created by Yash soni on 09/08/23.
//
...
{
    vlan_8021q_hdr_t *vlan_8021q_hdr = (vlan_8021q_hdr_t *)((char *)ethernet_hdr + (2 * sizeof(mac_add_t)));
...
is_tagged_arp_broadcast_request_msg(ethernet_hdr_t *ethernet_hdr)
{
    vlan_ethernet_hdr_t *vlan_ethernet_hdr = (vlan_ethernet_hdr_t *)ethernet_hdr;
    
    if(IS_MAC_BROADCAST(vlan_ethernet_hdr->dst_mac.mac))
        return TRUE;
    else if(vlan_ethernet_hdr->vlan_8021q_hdr.tci_vid >= 10 && vlan_ethernet_hdr->vlan_8021q_hdr.tci_vid <= 20)
        return TRUE;
                                   ...
                                   unsigned int *output_vlan_id)
{
    ...
    if(IS_INTF_L3_MODE(interface) &&
       memcmp(IF_MAC(interface),eth_hdr->dst_mac.mac,sizeof(mac_add_t)) == 0)
        return TRUE;
